...
####Make sure you load any necessary libraries OK

####HINT: Use a filter command to get just maps of Costa Rica, Panama, and Nicaragua OK

####Hint: Use a filter command to put in points only for cities with a population of greater than 40,000. This should leave you with 32 cities.OK
...
####Hint: set the size of all points to the value 5 OK
...
dat <- filter(my_world_map,region=="Costa Rica" | region=="Panama" | region=="Nicaragua")

my_cities <-maps::world.cities

dat_cities <- filter(my_cities, country.etc=="Costa Rica" & pop>40000 | country.etc=="Panama" & pop>40000 | country.etc=="Nicaragua" & pop>40000)

dat_cities <- add_column(dat_cities, Measurement)

ggplot(data = dat, mapping = aes(x= long, y= lat, group = group))+
  geom_polygon(color="black",fill="white")+
  geom_point(data = dat_cities,aes(x=long,y=lat,group=NULL,color=Measurement),size=5)+
